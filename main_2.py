...
def train_one_epoch(args, epoch, model, predictor, barlow_twins_loss, data_loader,
    ...
    for iteration, (images, _) in enumerate(train_bar):
        ...
        #print(f"the shape of spec is {images[0].shape}")# post-normalization block from BYOL-A [Niizumi et al., 2021]
        if args.post_norm:
            norm_images = []
            for im in images:
        ...
        if logger is not None:
            logger.info('epoch,{},step,{},loss,{},forward_time,{},backward_time,{}, data_time, {}'.format(
                        epoch, iteration, total_loss / total_num, forward_time, backward_time, data_time))
        if wandb_run is not None:
            wandb_run.log({'Loss': total_loss / total_num, 'forward_time':forward_time, 'backward_time': backward_time, "data_time":data_time})
            if args.masked_recon:
                wandb_run.log({
...
if __name__ == '__main__':
    ...
    save_name = f"rrc_{args.RRC}_rlf_{args.RLF}_mix_{args.mixup}_noise_{args.Gnoise}_mp3_{args.mp3_compression}_ldm_{args.ldm_compression}_mix_{args.mixed_compression}_mix2_{args.mixed_compression2}_lrate_{args.ldm_rate}"
    save_name += timestamp
    ...
    # logging
    if utils.is_main_process():
        log_dir = f"/rds/general/user/zw1222/ephemeral/logs/training/{args.dataset}/{save_name}/"
