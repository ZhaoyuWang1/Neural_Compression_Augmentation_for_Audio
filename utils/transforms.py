...
class AudioPairTransform(nn.Module):
	...
	def __init__(self, args, train_transform=True, multi_transform=True, 
		...
		super().__init__()
		self.multi_transform = multi_transform if (not args.mp3_compression) and (not args.ldm_compression) and (not args.mixed_compression) and (not args.mixed_compression2) else False
